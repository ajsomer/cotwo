...
export interface BuiltIntakePdf {
  buffer: Buffer;
  fileName: string;
  patientName: string;
}
...
/**
 * Render the whole intake package (every configured form + card/consent
 * summary) as a single PDF buffer. Shared by the inline PDF route and the
 * "attach to PMS" push. Returns null when there's no intake_package action.
 */
export async function buildIntakePackagePdf(
  appointmentId: string
): Promise<BuiltIntakePdf | null> {
  ...
  return {
    ...
    fileName: intakePackagePdfFilename(patientName, completedAt),
    patientName,
  };
}
